...
public class SimpleMonthView extends View {
    ...
    private static final int UNAVAILABLE_CIRCLE_ALPHA = 130;
    ...
    protected final List<Date> listDateUnavailable;
    protected final int mUnavailableNumberColor;
    ...
    private Paint mUnAvailableCirclePain;
    private Calendar calendar;

    public SimpleMonthView(Context context, List<Date> listDateUnavailable) {
        super(context);
        this.listDateUnavailable = listDateUnavailable;
        ...
        mUnavailableNumberColor = resources.getColor(R.color.un_available_date);
    ...
    private boolean sameDay(int monthDay, Date date) {
        if (calendar == null) {
            calendar = Calendar.getInstance();
        }
        calendar.setTime(date);
        return (mYear == calendar.get(Calendar.YEAR)) && (mMonth == calendar.get(Calendar.MONTH)) && (monthDay == calendar.get(Calendar.DAY_OF_MONTH));
    }
    ...
    protected void drawMonthNums(Canvas canvas) {
        ...
        int day = 1;
        boolean selectedDateIsUnavailableDate = isUnavailableDate(mSelectedDay);

        while (day <= mNumCells) {
            int x = paddingDay * (1 + dayOffset * 2) + mPadding;
            if (!selectedDateIsUnavailableDate && mSelectedDay == day) {
                canvas.drawCircle(x, y - MINI_DAY_NUMBER_TEXT_SIZE / 3, DAY_SELECTED_CIRCLE_SIZE, mSelectedCirclePaint);
            }
            if (isUnavailableDate(day)) {
                canvas.drawCircle(x, y - MINI_DAY_NUMBER_TEXT_SIZE / 3, DAY_SELECTED_CIRCLE_SIZE, mUnAvailableCirclePain);
            }
    ...
    private boolean isUnavailableDate(int day) {
        if (listDateUnavailable == null) {
            return false;
        }
        for (Date date : listDateUnavailable) {
            if (sameDay(day, date)) {
                return true;
            }
        }
    ...
    protected void initView() {
        ...
        mUnAvailableCirclePain = new Paint();
        mUnAvailableCirclePain.setFakeBoldText(true);
        mUnAvailableCirclePain.setAntiAlias(true);
        mUnAvailableCirclePain.setColor(mUnavailableNumberColor);
        mUnAvailableCirclePain.setTextAlign(Align.CENTER);
        mUnAvailableCirclePain.setStyle(Style.FILL);
        mUnAvailableCirclePain.setAlpha(UNAVAILABLE_CIRCLE_ALPHA);
